...
export class PanelLayoutManager implements AppModule {
  ...
  private attachGoeTabListeners(): void {
    const kgBtn = document.getElementById('goeTabKg');
    const mapBtn = document.getElementById('goeTabMap');
    const kgView = document.getElementById('goeKgView');
    const mapSection = document.getElementById('mapSection');
    const panelsGrid = document.getElementById('panelsGrid');

    if (!kgBtn || !mapBtn || !kgView || !mapSection || !panelsGrid) return;

    kgBtn.addEventListener('click', () => {
      kgBtn.classList.add('active');
      mapBtn.classList.remove('active');
      kgView.classList.remove('goe-tab-hidden');
      mapSection.classList.add('goe-tab-hidden');
      panelsGrid.classList.add('goe-tab-hidden');
      // Trigger map resize check just in case
    ...
    mapBtn.addEventListener('click', () => {
      ...
      kgView.classList.add('goe-tab-hidden');
      mapSection.classList.remove('goe-tab-hidden');
      panelsGrid.classList.remove('goe-tab-hidden');
      // Trigger map resize to fix layout
      window.dispatchEvent(new Event('resize'));
      setTimeout(() => this.ctx.map?.resize(), 150);
    });
  }
    ...
    this.ctx.container.innerHTML = `
      ...
      <div class="header">
        ...
        return `
          ...
          ${SITE_VARIANT === 'goe'
        ? `<span class="logo" style="color: var(--goe-teal, #00d4aa);">GOE</span><span class="logo-mobile" style="color: var(--goe-teal, #00d4aa);">GOE</span><span class="version" style="font-family: monospace; letter-spacing: 1px;">GLOBAL ONTOLOGY ENGINE</span>`
        ...
        <div class="header-right">
          ...
          <button class="search-btn" id="searchBtn" style="${SITE_VARIANT === 'goe' ? 'display: none;' : ''}"><kbd>⌘K</kbd> ${t('header.search')}</button>
          ${this.ctx.isDesktopApp ? '' : `<button class="copy-link-btn" id="copyLinkBtn" style="${SITE_VARIANT === 'goe' ? 'display: none;' : ''}">${t('header.copyLink')}</button>`}
          <button class="theme-toggle-btn" id="headerThemeToggle" title="${t('header.toggleTheme')}" style="${SITE_VARIANT === 'goe' ? 'display: none;' : ''}">
            ${getCurrentTheme() === 'dark'
        ? '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>'
        : '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z"/></svg>'}
          </button>
          ${this.ctx.isDesktopApp ? '' : `<button class="fullscreen-btn" id="fullscreenBtn" title="${t('header.fullscreen')}" style="${SITE_VARIANT === 'goe' ? 'display: none;' : ''}">⛶</button>`}
          ${SITE_VARIANT === 'happy' ? `<button class="tv-mode-btn" id="tvModeBtn" title="TV Mode (Shift+T)"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg></button>` : ''}
          <span id="unifiedSettingsMount"></span>
        </div>
      </div>
      ${SITE_VARIANT === 'goe' ? `
      <div class="goe-domain-filters" id="goeDomainFilters" style="display: flex; gap: 8px; padding: 4px 16px; background: rgba(0,0,0,0.2); overflow-x: auto; border-bottom: 1px solid rgba(255,255,255,0.05); -ms-overflow-style: none; scrollbar-width: none;">
      </div>
      ` : ''}
      ...
      <div class="main-content">
        ${SITE_VARIANT === 'goe' ? `
        <div class="goe-tab-bar">
          <button class="goe-tab-btn active" id="goeTabKg">Knowledge Graph</button>
          <button class="goe-tab-btn" id="goeTabMap">Situation Map</button>
        </div>
        <div class="goe-dashboard-view" id="goeKgView">
          <div class="goe-kg-main" id="panel-goe_knowledge_graph"></div>
          <div class="goe-side-drawer">
            <div class="goe-side-section">
              <div class="goe-side-header">INTELLIGENCE FEED</div>
              <div class="goe-side-content" id="slot-goe_domain_feed"></div>
            </div>
            <div class="goe-side-section">
              <div class="goe-side-header">STRATEGIC BRIEF</div>
              <div class="goe-side-content" id="slot-ai_brief"></div>
            </div>
          </div>
        </div>
        <div class="map-section goe-tab-hidden" id="mapSection">
          <div class="panel-header" style="background: rgba(10,22,40,0.95); border-bottom: 1px solid rgba(0,212,170,0.15);">
            <div class="panel-header-left">
              <span class="panel-title" style="color: #00d4aa; font-weight: 800; letter-spacing: 2px; font-size: 11px;">SITUATION MAP</span>
            </div>
        ...
        <div class="panels-grid goe-tab-hidden" id="panelsGrid"></div>
        ` : `
  ...
  private initGoeFilters(): void {
    const container = document.getElementById('goeDomainFilters');
    if (!container) return;

    const domains = ['geopolitics', 'economics', 'defence', 'technology', 'climate', 'society'];
    let activeDomains = new Set(domains);
    try {
      const stored = localStorage.getItem('goe-active-domains');
      if (stored) activeDomains = new Set(JSON.parse(stored));
    } catch { }
    ...
    const render = () => {
      ...
      domains.forEach(domain => {
        ...
        btn.style.cssText = `
          ...
          letter-spacing: 1px;
          cursor: pointer;
          border: 1px solid ${isActive ? 'var(--goe-teal, #00d4aa)' : 'rgba(255,255,255,0.2)'};
          background: ${isActive ? 'var(--goe-teal, #00d4aa)' : 'transparent'};
          color: ${isActive ? '#050c14' : 'rgba(255,255,255,0.6)'};
          transition: all 0.2s;
        `;
        btn.onclick = () => {
          if (activeDomains.has(domain)) activeDomains.delete(domain);
          else activeDomains.add(domain);
          localStorage.setItem('goe-active-domains', JSON.stringify(Array.from(activeDomains)));
          window.dispatchEvent(new CustomEvent('goe:domain-filter-change', { detail: { activeDomains } }));
          render();
        };
  ...
  private createPanels(): void {
    ...
    if (SITE_VARIANT === 'full' || SITE_VARIANT === 'goe') {
    ...
    if (SITE_VARIANT === 'goe') {
      const goeDomainFeed = new GoeDomainFeed();
      this.ctx.panels['goe_domain_feed'] = goeDomainFeed;

      const goeKnowledgeGraph = new GoeKnowledgeGraph();
      this.ctx.panels['goe_knowledge_graph'] = goeKnowledgeGraph;
      void goeKnowledgeGraph.refresh();
    }
    ...
    if (SITE_VARIANT === 'goe') {
      const kgSlot = document.getElementById('panel-goe_knowledge_graph');
      const feedSlot = document.getElementById('slot-goe_domain_feed');
      const briefSlot = document.getElementById('slot-ai_brief');

      if (kgSlot && this.ctx.panels['goe_knowledge_graph']) {
        kgSlot.appendChild(this.ctx.panels['goe_knowledge_graph'].getElement());
      }
      if (feedSlot && this.ctx.panels['goe_domain_feed']) {
        feedSlot.appendChild(this.ctx.panels['goe_domain_feed'].getElement());
        void (this.ctx.panels['goe_domain_feed'] as any).refresh?.();
      }
      if (briefSlot && this.ctx.panels['insights']) {
        briefSlot.appendChild(this.ctx.panels['insights'].getElement());
        void (this.ctx.panels['insights'] as any).updateInsights?.(this.ctx.latestClusters);
      }
      this.attachGoeTabListeners();
    }
